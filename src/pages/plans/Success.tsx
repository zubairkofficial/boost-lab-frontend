...
export default function Success() {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/plans/verify-payment/${sessionId}`);
    ...
    return (
      ...
      >
        <div className="bg-[#154E62]/80 backdrop-blur-lg border border-cyan-400/30 shadow-2xl text-white px-10 py-12 rounded-xl text-center max-w-xl w-full">
          ...
          <h1 className="text-2xl font-bold text-cyan-300 mb-4">Verifying Payment...</h1>
          <p className="text-slate-200">Please wait while we confirm your payment.</p>
        </div>
      </div>
    );
  }
